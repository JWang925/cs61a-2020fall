...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    print("DEBUG: calling take_turn. num_rolls=", num_rolls, ", oppo_score:", opponent_score)
    if num_rolls == 0:
...
def swine_align(player_score, opponent_score):
    ...
    while i <= min(player_score, opponent_score):
        if (player_score % i == 0) & (opponent_score % i == 0):
            gcd = i
        i += 1
    if gcd >= 10:
        return True
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while end_game == False:
        if who == 0:
            n_roll = strategy0(score0, score1)
            score0 += take_turn(n_roll, score1, dice)
            print("DEBUG: score 0:", score0)
            if not extra_turn(score0, score1):
                who = other(who)
        else:
            n_roll = strategy1(score1, score0)
            score1 += take_turn(n_roll, score0, dice)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    "*** YOUR CODE HERE ***"
    if who == 0:
        def say(score1, score2):
            if score1 - last_score > running_high:
                print(score1 - last_score ,"point(s)! The most yet for Player", who)
                return announce_highest(who, score2, score1 - last_score)
            return announce_highest(who, score2, running_high)
    else:
        def say(score1, score2):
            if score2 - last_score > running_high:
                print(score2 - last_score ,"point(s)! The most yet for Player", who)
                return announce_highest(who, score2, score2 - last_score)
            return announce_highest(who, score2, running_high)
    return say
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    i_num_dice, best_num_dice = 1, 1
    best_avg_roll_dice = 0
    while i_num_dice <= 10:
        avg = averaged_roll_dice(i_num_dice, dice)
        if avg > best_avg_roll_dice:
            best_num_dice = i_num_dice
            best_avg_roll_dice = avg
        i_num_dice += 1
